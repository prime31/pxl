...
foreign libc {
	...
	pxl_draw_text_len :: proc(text: rawptr, text_len: uint, x, y: f32, color: Color) ---
...
Callbacks :: struct {
	setup, update, render, shutdown: #type proc(),
}
Pass :: struct {
	clear_color_value:                                  u32,
	has_clear_color, has_camera:                        bool,
	cam_offset_x, cam_offset_y, cam_zoom, cam_rotation: f32,
	pixel_snap:                                         bool,
}

color_rgba :: proc(r, g, b, a: u8) -> Color {return Color(
		u32(r) | u32(g) << 8 | u32(b) << 16 | u32(a) << 24,
	)}
color_rgb :: proc(r, g, b: u8) -> Color {return color_rgba(r, g, b, 255)}
begin_pass :: proc(clear: Color) {pxl_pass_begin(
		Pass{clear_color_value = u32(clear), has_clear_color = true, pixel_snap = true},
	)}
end_pass :: proc() {pxl_pass_end()}
rect :: proc(pos, size: Vec2, color: Color) {pxl_draw_rect(pos.x, pos.y, size.x, size.y, color)}
circle :: proc(center: Vec2, radius: f32, segments: u32, color: Color) {pxl_draw_circle(
		center.x,
		center.y,
		radius,
		segments,
...
line :: proc(a, b: Vec2, thickness: f32, color: Color) {pxl_draw_line(
		a.x,
		a.y,
		b.x,
		b.y,
...
text :: proc(value: string, pos: Vec2, color: Color) {pxl_draw_text_len(
		raw_data(value),
		uint(len(value)),
		pos.x,
		pos.y,
...
textured_rect :: proc(value: ^Texture, dst, src: Rect, color: Color) {pxl_draw_textured_rect(
		value,
		dst.x,
		dst.y,
		dst.w,
...
add_binding :: proc(action: string, key: Keycode) {pxl_input_add_binding(
		raw_data(action),
		uint(len(action)),
		i32(key),
	)}
get_vector :: proc(
	neg_x, pos_x, neg_y, pos_y: string,
	diagonal: Axis_Diagonal,
) -> Vec2 {return pxl_input_get_vector(
		raw_data(neg_x),
		uint(len(neg_x)),
		raw_data(pos_x),
		uint(len(pos_x)),
...
load_texture :: proc(path: string) -> ^Texture {return pxl_assets_load_texture_path(
		raw_data(path),
		uint(len(path)),
	)}
load_aseprite :: proc(path: string) -> ^Texture {return pxl_aseprite_load_path(
		raw_data(path),
		uint(len(path)),
	)}
find_aseprite :: proc(path: string) -> u32 {return pxl_aseprite_find_id(
		raw_data(path),
		uint(len(path)),
	)}
aseprite_anim :: proc(tag: u32) -> u32 {return pxl_aseprite_tag_anim(tag)}
aseprite_anim_by_name :: proc(id: u32, name: string) -> u32 {return pxl_aseprite_anim_by_name(
		id,
		raw_data(name),
		uint(len(name)),
	)}
destroy_texture :: proc(texture: ^Texture) {if texture != nil {pxl_assets_destroy_texture(
			texture,
		)}}
...
anim_current_frame :: proc(
	player: ^Anim_Player,
	texture: ^^Texture,
	src: ^Rect,
	color: ^Color,
) -> bool {flip_x, flip_y: bool; return pxl_anim_player_current_frame(
		player,
		texture,
		&src.x,
		&src.y,
