...
fn generateAssetCHeader(b: *Build, entries: *const std.ArrayList(AssetEntry)) ![]const u8 {
    ...
    const kinds = [_]struct { prefix: []const u8, kind: AssetKind, counter: u32 = 0 }{
        .{ .prefix = "PXL_TEXTURE_", .kind = .texture },
        .{ .prefix = "PXL_FONT_", .kind = .font },
        .{ .prefix = "PXL_TILEMAP_", .kind = .tilemap },
        .{ .prefix = "PXL_AUDIO_", .kind = .audio },
    };
    ...
    for (&kinds) |*k| {
        ...
        for (entries.items) |e| {
            if (e.kind != k.kind) continue;
            // Uppercase the id_name for C convention.
            ...
            const upper_name = name_buf[0..j];
            try buf.appendSlice(b.allocator, b.fmt("#define {s}{s} {d}\n", .{ k.prefix, upper_name, idx }));
            idx += 1;
        }
        try buf.appendSlice(b.allocator, "\n");
    }
...
pub fn build(b: *Build) !void {
    ...
    if (!target.result.cpu.arch.isWasm()) {
        ...
        // ── Static library + C header ───────────────────────────────────────
        // Build the static lib from the pxl module directly, with c_entrypoint
        // as the root so the C API exports are linked in.
